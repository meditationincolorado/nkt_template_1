...
;(function($) {
    ...
    $(function() {
        ...
        if (_window.width() > 781) {
            ...
            if (body.is('.header-image')) {
                toolbarOffset = body.is('.admin-bar')
                    ? $('#wpadminbar').height()
                    : 0
                ...
                _window.on('scroll.twentyfourteen', function() {
                    if (
                        _window.scrollTop() > mastheadOffset &&
                        mastheadHeight < 49
                    ) {
                    ...
                    function(placement) {
                        ...
                        if (copyPosition) {
                            placement.container.css({
                                position: placement.removedNodes.css(
                                    'position'
                                ),
    ...
    _window.on('scroll', function() {
        var $bannerCTAhidden = $bannerCTA.hasClass('hidden'),
            $bannerOffsetHeight = $bannerCTAhidden
                ? 0
                : $bannerCTA.outerHeight()
        ...
        if (!playingMeditation) {
            ...
            var interval = setInterval(function() {
                var timeLeftinSeconds =
                        meditationDurationInSeconds - timeElapsed,
        ...
        if ($headers.length > 0) {
                ...
                if ($subHeaders.length > 0) {
                    ...
                    $subHeaders.each(function() {
                        var $subHeader = $(this),
                            ...
                            $articleParent = $subHeader.closest(
                                'section[data-num]'
                            ),
                            isActive = subHeaderCt == 0 ? ' active' : '',
                            bullet =
                                subHeaderCt == $subHeaders.length - 1
                                    ? ''
                                    : ' &bull; ',
                            subTabText =
                                '<a class="sub-tab' +
                        ...
                        if ($articleParent.find('.sub-tabs').length == 0) {
                            $articleParent.prepend(
                                '<span class="sub-tabs"></span>'
                            )
                        }
            ...
            $tabs.on('click', function() {
                var $tab = $(this),
                    ...
                    $content = $navSection.find(
                        'section[data-num="' + $tabNum + '"]'
                    ),
            ...
            $subTabs.on('click', function() {
                var $tab = $(this),
                    $tabNum = $tab.attr('data-num'),
                    $article = $navSection.find(
                        'article[data-num="' + $tabNum + '"]'
                    ),
    ...
    if ($calendar.length) {
        ...
        for (i = 0; i < 7; i++) {
            if (
                firstDay.toString().substring(0, 3) == days[i].substring(0, 3)
            ) {
                offset = i
                break
            }
        ...
        for (i = 0; i < 42; i++) {
            var dayNum =
                i < offset
                    ...
                    : (i - offset) % 31 + 1
            var otherClass =
                i < offset || i - offset > daysInMonth - 1 ? 'other' : ''
    ...
    $homeForms.submit(function() {
        ...
        if (cookieName == $convertedUser) {
            setCookie(
                'user-firstname',
                $homeForms.find('[name=firstname]').val()
            )
